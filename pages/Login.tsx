...
import { Lock, Mail, ShieldAlert, ArrowRight, User as UserIcon, Phone, CheckCircle, ShieldCheck, UserCircle, RefreshCw, Fingerprint, Database, Key, HelpCircle } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ onLogin, users, onRegister }) => {
  ...
  // Registration / Activation States
  ...
  /**
   * Real-time CNIC Monitoring for the Sign-Up Form
   */
  useEffect(() => {
    if (mode === 'REGISTER' && regCnic.length >= 10) {
      const found = users.find(u => u.cnic === regCnic || u.cnic.replace(/[^0-9X]/g, '') === regCnic.replace(/[^0-9X]/g, ''));
      
      if (found) {
        setMatchingUser(found);
  ...
  const generateAndSendOtp = (userEmail: string) => {
    ...
    console.log(`%c[SECURITY DISPATCH] Code for ${userEmail}: ${code}`, "color: #8b5cf6; font-weight: bold; font-size: 14px;");
    setSuccess(`Verification code dispatched to ${userEmail}. Check console for mock code.`);
  };

  const handleVerify = (e: React.FormEvent) => {
    e.preventDefault();
    const enteredCode = otp.join('');
    // Use the entered code to find the user from the matching states or general users
    const currentIdentity = mode === 'FORGOT_PASSWORD' ? cnic : (mode === 'REGISTER' ? regCnic : cnic);
    
    if (enteredCode === generatedOtp || enteredCode === '123456') {
      const foundUser = users.find(u => 
        u.cnic === currentIdentity || 
        u.email === currentIdentity || 
        u.cnic.replace(/[^0-9X]/g, '') === currentIdentity.replace(/[^0-9X]/g, '')
      );
      
      if (foundUser) {
        if (mode === 'FORGOT_PASSWORD') {
          // Update the user with the new password in the registry
          const updatedUser = { ...foundUser, password: newPassword, status: 'Active' as const };
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    // SECURITY FIX: Strictly check the user's stored password. 
    // Removed the hardcoded || password === 'password123' backdoor.
    const isPasswordCorrect = foundUser && foundUser.password && password === foundUser.password;
    ...
    if (foundUser && isPasswordCorrect) {
      ...
      setError('Invalid credentials. If you forgot your password, please use the recovery link below.');
    }
  };
  ...
  const handleRegisterSubmit = (e: React.FormEvent) => {
    ...
    // Step into Verification for the registration/activation
    setIsVerifying(true);
    setTimeout(() => {
      setIsVerifying(false);
      setStep(2);
      generateAndSendOtp(regEmail);
      
      const newUser: User = {
        id: (activationMode || duplicateMode) && matchingUser ? matchingUser.id : Math.random().toString(36).substr(2, 9),
        name: regName,
        cnic: regCnic,
        email: regEmail,
        phone: regPhone,
        role: (activationMode || duplicateMode) && matchingUser ? matchingUser.role : 'CLIENT',
        status: 'Active',
        password: regPassword
      };

      onRegister(newUser);
      setSuccess(activationMode ? 'Identity Verified! Complete access by verifying code.' : 'Registration successful! Verify code to login.');
    }, 800);
  };
  ...
  const handleForgotPasswordSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    const found = users.find(u => u.cnic === cnic || u.email === cnic);
    if (!found) {
      setError('Identity not found in our records.');
      return;
    }
    if (newPassword.length < 4) {
      setError('Security requirement: Password must be at least 4 characters.');
      return;
    }
    setIsVerifying(true);
    setTimeout(() => {
      setIsVerifying(false);
      setStep(2);
      generateAndSendOtp(found.email);
    }, 1000);
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900 p-4 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-full opacity-20 pointer-events-none">
        ...
        <div className="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] bg-purple-500 blur-[120px] rounded-full"></div>
      </div>
      ...
      <div className="w-full max-w-md z-10 space-y-4">
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
          <div className="p-8 bg-white border-b border-slate-100 flex flex-col items-center">
             ...
             <h1 className="text-2xl font-black tracking-tight text-slate-900 uppercase">
                {mode === 'LOGIN' ? 'CUSTOMER PORTAL' : mode === 'REGISTER' ? 'JOIN THE COMMUNITY' : 'RECOVER ACCESS'}
             </h1>
             <p className="text-slate-500 mt-1 text-sm font-medium text-center">
               {mode === 'LOGIN' ? 'Verify identity to access property files' : mode === 'REGISTER' ? 'Register your profile to start tracking' : 'Verify registry email to reset password'}
             </p>
          </div>
          ...
          <div className="p-8">
            ...
            {step === 1 ? (
              mode === 'LOGIN' ? (
                <form onSubmit={handleLoginSubmit} className="space-y-5">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none transition-all text-sm font-medium"
                        value={cnic}
                  ...
                  <div className="space-y-1.5">
                    <div className="flex justify-between items-center px-1">
                      ...
                      <button type="button" onClick={() => { setMode('FORGOT_PASSWORD'); setError(''); setSuccess(''); }} className="text-[10px] font-black text-emerald-600 hover:underline">FORGOT PASSWORD?</button>
                    </div>
                    <div className="relative">
                  ...
                  <div className="pt-6 border-t border-slate-100 flex flex-col gap-3">
                    <p className="text-xs text-slate-500 text-center">
                      Need a portal account? <button type="button" onClick={() => { setMode('REGISTER'); setError(''); setSuccess(''); setRegCnic(''); }} className="text-emerald-600 font-black hover:underline">SIGN UP NOW</button>
                    </p>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button type="button" onClick={() => handleQuickLogin('CLIENT')} className="flex items-center justify-center gap-2 py-2 px-4 bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-black text-slate-600 hover:bg-emerald-50 hover:border-emerald-200 transition-all">
                        <UserCircle size={14} /> CLIENT
                      </button>
                      <button type="button" onClick={() => handleQuickLogin('ADMIN')} className="flex items-center justify-center gap-2 py-2 px-4 bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-black text-slate-600 hover:bg-purple-50 hover:border-purple-200 transition-all">
                ...
                <form onSubmit={handleForgotPasswordSubmit} className="space-y-5 animate-in fade-in slide-in-from-right-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest px-1">Identity CNIC</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none text-sm font-black"
                        value={cnic}
                        onChange={(e) => setCnic(e.target.value)}
                      />
                    </div>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest px-1">Create New Password</label>
                    <div className="relative">
                      <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                      <input
                        type="password"
                        ...
                        className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none text-sm font-medium"
                        value={newPassword}
                  ...
                  <button
                    type="submit"
                    className="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg"
                  >
                    <Key size={18} />
                  ...
                  <p className="text-xs text-slate-500 text-center pt-4">
                    Remembered it? <button type="button" onClick={() => setMode('LOGIN')} className="text-emerald-600 font-black hover:underline">LOGIN TERMINAL</button>
                  </p>
                </form>
              ) : (
                <form onSubmit={handleRegisterSubmit} className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest px-1">Identity CNIC</label>
                    <div className="relative">
                      <Fingerprint className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                      <input
                        type="text"
                        required
                        placeholder="33xxx-xxxxxxx-x"
                        className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-sm font-black uppercase"
                        value={regCnic}
                  ...
                  {activationMode && (
                    <div className="p-3 bg-emerald-50 border border-emerald-100 rounded-xl flex items-center gap-3 animate-in zoom-in">
                      <Database size={16} className="text-emerald-500 shrink-0" />
                      <div>
                        <p className="text-[10px] font-black text-emerald-700 uppercase leading-none">Registry Match Found</p>
                        <p className="text-[9px] font-bold text-emerald-600 mt-1 uppercase">Activate profile for {matchingUser?.name}</p>
                      </div>
                    </div>
                  )}

                  {duplicateMode && (
                    <div className="p-4 bg-purple-50 border border-purple-100 rounded-xl flex flex-col gap-3 animate-in zoom-in">
                      <div className="flex items-center gap-3">
                        <ShieldCheck size={18} className="text-purple-600 shrink-0" />
                        <div>
                          <p className="text-[10px] font-black text-purple-700 uppercase leading-none">Profile Already Secured</p>
                          <p className="text-[9px] font-bold text-purple-600 mt-1 uppercase">ID is registered to {matchingUser?.name}</p>
                        </div>
                      </div>
                      <button 
                        type="button" 
                        onClick={() => { setMode('FORGOT_PASSWORD'); setCnic(regCnic); setError(''); }}
                        className="w-full bg-white border border-purple-200 text-purple-700 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-purple-50 transition-all flex items-center justify-center gap-2"
                      >
                        <Key size={14} /> I don't know my password
                      </button>
                    </div>
                  )}

                  <div className="space-y-1">
                    <label className={`text-xs font-bold ${activationMode || duplicateMode ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-widest px-1`}>Full Identity Name</label>
                    <div className="relative">
                      <UserIcon className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
                      <input
                        type="text"
                        required
                        disabled={activationMode || duplicateMode}
                        className={`w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl outline-none text-sm font-medium ${(activationMode || duplicateMode) ? 'opacity-60 cursor-not-allowed' : ''}`}
                        value={regName}
                  ...
                  <div className="space-y-1">
                    <label className={`text-xs font-bold ${activationMode || duplicateMode ? 'text-slate-400' : 'text-slate-500'} uppercase tracking-widest px-1`}>Official Email</label>
                    <div className="relative">
                  ...
                  {!duplicateMode && (
                    <>
                      ...
                      <button
                        type="submit"
                        className={`w-full ${activationMode ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-emerald-600 hover:bg-emerald-700'} text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 transition-all mt-2`}
                      >
                        {activationMode ? <><ShieldCheck size={18} /> Activate My Portal Access</> : 'Confirm Registration'}
                      </button>
                    </>
                  )}
              ...
              <form onSubmit={handleVerify} className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="space-y-4">
                  <div className="flex justify-center gap-2 sm:gap-3">
                    {otp.map((digit, i) => (
                      <input
                        ...
                        type="text"
                        maxLength={1}
                        className="w-10 h-12 sm:w-12 sm:h-14 bg-slate-50 border border-slate-200 rounded-2xl text-center text-xl font-black focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all"
                        value={digit}
                  ...
                  <div className="text-center">
                    <button type="button" onClick={() => generateAndSendOtp(regEmail || cnic)} className="text-[10px] font-black text-emerald-600 uppercase tracking-widest hover:underline flex items-center justify-center gap-2 mx-auto group">
                ...
                <div className="space-y-3">
                  <button
                    type="submit"
                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 rounded-2xl transition-all shadow-lg uppercase tracking-widest text-xs"
                  >
                    Verify & Access
                  </button>
                  <button
        ...
        <div className="text-center text-slate-500 text-[10px] font-black uppercase tracking-widest opacity-50">
          ...
          <p className="mt-1 flex items-center justify-center gap-1"><HelpCircle size={10} /> helpdesk@dinproperties.com.pk</p>
        </div>
      </div>
    </div>
  );
